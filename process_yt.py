...
sentences, sen_test = [], []
# 读取data_tag作为训练数据
text = open('./data/yt_data/data_tag_test', encoding='utf-8').read().split('\n')  # 消除换行符
# 读取训练文本数据
train_text = open('./data/yt_data/data_train_text', encoding='utf-8').read().split('\n')
# 读取data_test作为测试数据
test_text = open('./data/yt_data/data_test', encoding='utf-8').read().split('\n')   #单独使用测试文本
# 读取data_dev作为验证数据
dev_test = open('./data/yt_data/data_dev', encoding='utf-8').read().split('\n')
# 读取不带标签的验证数据
dev_text_nolab = open('./data/yt_data/data_dev_text', encoding='utf-8').read().split('\n')
# 读取不带标签的test文本
test_text_nolab = open('./data/yt_data/data_test_text', encoding='utf-8').read().split('\n')
# 读取词表
words_table = open('./data/yt_data/words_table', encoding='utf-8').read().split('\n')
# 获取正确语料
...
sentences_right = open('./data/yt_data/data_text_noRepeat')
...
with open(join(path, 'data_word3vec_yt.pkl'), 'wb') as f:
