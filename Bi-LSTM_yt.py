...
# 对准确率画折线图
def accuracy_line(x_log, y_value):
    plt.plot(x_log, y_value)
    plt.axis([0, 3, 0.5, 1])
    colors1 = '#6D6D6D'  # 设置标题颜色为灰色
    plt.title('the accuracy for each group', color=colors1, fontsize=18)
    plt.xlabel('group')
    plt.ylabel(u'%')
    plt.show()
...
def main():
    ...
    with tf.name_scope('hidden2'):
        ...
        hidden1 = tf.multiply(hidden, word_weight)  # 将决策权重矩阵与bi-lstm相结合
    ...
    if FLAGS.train:
        ...
        ckpt = tf.train.get_checkpoint_state('ckpt3')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_batch_size', help='test batch size', default=120)  # 模型每次测试的样本
    parser.add_argument('--source_data', help='source size', default='./data/data_word3vec_yt.pkl')
    ...
    parser.add_argument('--checkpoint_dir', help='checkpoint dir', default='ckpt3/model.ckpt', type=str)
