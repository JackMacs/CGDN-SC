...
for filename in os.listdir(dirname):
    with open(os.path.join(dirname, filename), 'r', encoding='utf-8') as lcqmc:
        for line in lcqmc:
            linedict = eval(line)
            word = linedict['sentence1']
            poss = linedict['sentence2']
...
with open('../data/public_simple_data/data_text', 'r', encoding='utf-8') as f:
    for line in f:
...
texts13 = open(path13, 'r', encoding='utf-8').read().split('\n')
texts15 = open(path15, 'r', encoding='utf-8').read().split('\n')
texts_test = open(path_test, 'r', encoding='utf-8').read().split('\n')
...
for string in sentence:
    ...
    # print(str)
    words.append(str)
model = Word2Vec(words, size=300, window=4, min_count=1, sg=1, workers=2)
