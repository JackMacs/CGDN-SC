...
def load_data():
    ...
    with open(FLAGS.source_data, 'rb') as f:
        ...
        return data_x, data_y, test_x, dev_x, dev_y, texts, word2id, tag2id, id2tag, test_weight, train_weight, dev_weight, \
...
def main():
    # Load data13
    train_x, train_y, test_x, dev_x, dev_y, texts, word2id, tag2id, id2tag, test_weight, train_weight, dev_weight,\
    ...
    iterator_t = tf.data.Iterator.from_structure(test_dataset.output_types, test_dataset.output_shapes)
    ...
    with tf.name_scope('test_dataset_initial'):
        test_initializer = iterator_t.make_initializer(test_dataset)
    ...
    with tf.name_scope('hidden2'):
        ...
        hidden2 = tf.layers.batch_normalization(hidden1, training=is_train)
    ...
    if FLAGS.train:
        ...
        with open('data/public_data/test_pub_result.txt', 'a') as fr:
            for step in range(int(test_steps)):
                # x_results, y_predict_results, acc, y_label_results, yps = sess.run([x, y_predict, accuracy, y_label_reshape, y_],
                #                                                               feed_dict={keep_prob:FLAGS.keep_prob, is_train:True})
                x_results, y_predict_results, yps = sess.run([x, y_predict,  y_], feed_dict={keep_prob: FLAGS.keep_prob, is_train: True})
                ...
                yps = np.reshape(yps, (x_results.shape[0], x_results.shape[1], 3))
                print("yps_shape:", yps.shape)
                for i in range(len(x_results)):
                    print("x_results.len:", len(x_results))
                    ...
                    y_predict_result = list(filter(lambda x : x, y_predict_results[i]))
                    y_predict_text = ''.join(id2tag[y_predict_result].values)
                    ...
                    if "e" not in y_predict_text:
                        for j in range(50):
                            pa = pa + yp[j][2]
                        p = (pa * 1.0)/50.0
                        print(texts[index], "predict_text:", y_predict_text)
                        print("正确的概率:", p)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_batch_size', help='test batch size', default=826)  # 模型每次测试的样本
